...
class Layout:
    ...
    def filter_empty_bboxes(self, points, num_points=100, margin=0.15, inverse_rotation=False):
        filtered_bboxes = []
        for bbox in self.bboxes:
            ...
            # R maps local column vectors into world coordinates. For row
            # vectors the inverse is (world-center) @ R, not @ R.T. Opt in to
            # preserve historical checkpoints/configs and running evaluations.
            rotated_points = translated_points @ (rot_mat if inverse_rotation else rot_mat.T)
