...
class FinetuningArguments(SwanLabArguments):
    ...
    freeze_point_projector: bool = field(
        default=False,
        metadata={"help": "Whether or not to freeze the point projector."},
    )
    require_llm_only_training: bool = field(
        default=False,
        metadata={
            "help": (
                "Require point backbone/projector to be frozen while at least "
                "one non-point parameter remains trainable."
            )
        },
    )
    require_full_model_training: bool = field(
        default=False,
        metadata={
            "help": (
                "Require the point backbone, point projector, and non-point "
                "model parameters all to remain trainable."
            )
        },
    )
