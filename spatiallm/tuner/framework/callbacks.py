...
def _copy_wandb_history_to_checkpoint(wandb, args, max_step: int) -> None:
    source_id = args.wandb_copy_from_run_id
    if not source_id:
        return

    source_entity = args.wandb_copy_from_entity or args.wandb_entity
    source_project = args.wandb_copy_from_project or args.wandb_project
    if not source_entity or not source_project:
        raise ValueError(
            "W&B clean-copy requires source entity and project information."
        )

    source_path = f"{source_entity}/{source_project}/{source_id}"
    source_run = wandb.Api().run(source_path)
    rows: list[tuple[int, int, dict[str, Any]]] = []
    source_max_step: Optional[int] = None
    for row in source_run.scan_history(page_size=args.wandb_copy_page_size):
        trainer_step = row.get("train/global_step")
        if trainer_step is None:
            continue
        trainer_step = int(trainer_step)
        source_max_step = (
            trainer_step
            if source_max_step is None
            else max(source_max_step, trainer_step)
        )
        if trainer_step > max_step:
            continue

        payload = {}
        for key, value in row.items():
            if key.startswith("_") or not isinstance(value, Number):
                continue
            numeric = float(value)
            if math.isfinite(numeric):
                payload[key] = value
        if payload:
            rows.append((trainer_step, int(row.get("_step", 0)), payload))

    if source_max_step is None or source_max_step < max_step:
        raise RuntimeError(
            "W&B clean-copy source history is incomplete: "
            f"source={source_path}, source_max_train_global_step="
            f"{source_max_step}, required_checkpoint_step={max_step}. "
            "Repair/sync the source run before resuming training."
        )
    ...
    if wandb.run is not None:
        wandb.run.summary["clean_copy_from_run_id"] = source_id
        wandb.run.summary["clean_copy_source_path"] = source_path
        wandb.run.summary["clean_copy_max_step"] = max_step
        wandb.run.summary["clean_copy_rows"] = len(rows)
    logger.info_rank0(
        "Clean-copied W&B history: "
...
class LogCallback(TrainerCallback):
    ...
    def on_log(
        ...
        **kwargs,
        ...
        latest_history = state.log_history[-1]
        for key, value in latest_history.items():
            if key.startswith(
                (
                    "bbox/",
                    "eval_bbox/",
                    "bbox_point_alignment/",
...
class ReporterCallback(TrainerCallback):
    ...
    def __init__(
        ...
        self._wandb_history_copied = False
    ...
    def on_train_begin(
        ...
        **kwargs,
        ...
        if "wandb" in args.report_to:
            ...
            if args.wandb_copy_from_run_id and not self._wandb_history_copied:
                if state.global_step <= 0:
                    raise RuntimeError(
                        "wandb_copy_from_run_id requires a resumed local checkpoint."
                    )
                _copy_wandb_history_to_checkpoint(
                    wandb,
                    args,
                    max_step=int(state.global_step),
                )
                self._wandb_history_copied = True
