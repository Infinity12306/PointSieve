...
def load_model(
    ...
    for key in (
        ...
        config.point_config[key] = getattr(data_args, key)
    patch_config(config, model_args, init_kwargs, is_trainable)
    if data_args.bbox_regression_aux or data_args.bbox_point_alignment_aux:
        # Transformers 4.46 can leave newly introduced head parameters on the
        # meta device when loading an older checkpoint with low-memory loading.
    ...
    if not is_trainable:
        ...
        if finetuning_args.freeze_point_tower:
            model.point_backbone.eval()
        if finetuning_args.freeze_point_projector:
            model.point_proj.eval()
