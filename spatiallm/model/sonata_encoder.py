...
def batched_point_token_bbox_overlap_mask(
    grid_coord: torch.Tensor,
    token_batch: torch.Tensor,
    keep_bboxes: torch.Tensor,
    voxel_size: float,
) -> torch.Tensor:
    """Apply each sample's GT boxes only to its packed final point tokens."""
    if keep_bboxes.ndim != 3 or keep_bboxes.shape[-1] != 7:
        raise ValueError(
            "Packed point_token_keep_bboxes must have shape [B, K, 7], "
            f"got {tuple(keep_bboxes.shape)}."
        )
    if token_batch.shape != (grid_coord.shape[0],):
        raise ValueError(
            "Packed token batch ids must align with final grid coordinates."
        )
    ...
    for batch_index in range(keep_bboxes.shape[0]):
        sample_mask = token_batch == batch_index
        keep_mask[sample_mask] = point_token_bbox_overlap_mask(
            grid_coord[sample_mask],
            keep_bboxes[batch_index],
            voxel_size,
        )
    return keep_mask
...
class Sonata(PointModule, PyTorchModelHubMixin):
    ...
    def forward(self, data_dict):
        ...
        if keep_bboxes is not None:
            if keep_bboxes.ndim == 3:
                keep_mask = batched_point_token_bbox_overlap_mask(
                    point["grid_coord"],
                    token_batch,
                    keep_bboxes,
